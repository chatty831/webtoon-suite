...
def draw_text_on_image(
    ...
    font_path: str = "Wild-Words-Font-2/CC Wild Words Roman.ttf"
) -> None:
    """
    ...
    # Use the provided custom font or fall back to a default
    if font_path and os.path.exists(font_path):
        # Start with a reasonable default size
        pil_font_size = 20 if font_size is None else int(font_size * 20)
        try:
            font = ImageFont.truetype(font_path, size=pil_font_size)
    ...
    def test_text_fit(text, font_size, max_width, max_height):
        ...
        try:
            test_font = ImageFont.truetype(font_path, size=font_size) if font_path and os.path.exists(font_path) else ImageFont.load_default()
        except Exception:
    ...
    if font_size is not None:
        ...
        try:
            font = ImageFont.truetype(font_path, size=min_size) if font_path and os.path.exists(font_path) else ImageFont.load_default()
        except Exception:
            font = ImageFont.load_default()
    ...
    try:
        ...
        logger.error(f"Error applying patch: {e}, bbox:{[x,y,w,h]}, patch shape:{patch_cv.shape}, image shape:{image.shape}")
...
def process_images_with_translations(
    ...
    for (img_idx, bbox), text in zip(cutout_info, translations):
        ...
        logger.info(f"Drawing text on image {img_idx} at bbox {bbox[:2]} + {bbox[2:]}: '{text[:30]}{'...' if len(text) > 30 else ''}'")
        try:
            draw_text_on_image(modified_images[img_idx], text, bbox, allow_expansion=allow_expansion, font_size=font_size)
        except Exception as e:
...
async def translate_text_regions(images: List[np.ndarray], bbox_results: List[List]) -> List[np.ndarray]:
    ...
    translated_images = process_images_with_translations(
        images, 
        inference_results, 
        all_cutout_info,
        font_size=None
